...
def IoU(Yi,y_predi):
    ...
    for c in range(Nclass):
        ...
        #print("class {:02.0f}: #TP={:7.0f}, #FP={:7.0f}, #FN={:7.0f}, IoU={:4.3f}".format(c,TP,FP,FN,IoU))
            print("class (%2d) %12.12s: #TP=%7.0f, #FP=%7.0f, #FN=%7.0f, IoU=%4.3f" % (c/2, cfg.CLASS_NAMES[c],TP,FP,FN,IoU))
            IoUs.append(IoU)
    mIoU = np.mean(IoUs)
...
# Date:   11 Feb 2020
    ...
    # Block 5: 512, 512, 512
    x = Conv2D(256, (3, 3), activation='relu', padding='same', name='block5_conv1', data_format=IMAGE_ORDERING )(pool4)
    x = Conv2D(256, (3, 3), activation='relu', padding='same', name='block5_conv2', data_format=IMAGE_ORDERING )(x)
    x = Conv2D(256, (3, 3), activation='relu', padding='same', name='block5_conv3', data_format=IMAGE_ORDERING )(x)
