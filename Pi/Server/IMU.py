...
class IMU:
    ...
    def imuUpdate(self):
        ...
        self.roll =roll
        self.yaw = yaw
        return self.pitch,self.roll,self.yaw,ax,ay,az
...
if __name__ == '__main__':
    ...
    while True:
        try:
            ...
            for t in range(1):        
                time.sleep(1)
                accel_data=s.sensor.get_accel_data()  
                ax=s.kalman_filter_AX.kalman(accel_data['x']-s.Error_value_accel_data['x'])
                ay=s.kalman_filter_AY.kalman(accel_data['y']-s.Error_value_accel_data['y'])
                az=s.kalman_filter_AZ.kalman(accel_data['z']-s.Error_value_accel_data['z'])
                r,p,y,ax,ay,az= s.imuUpdate()
                d=dold+0.5*ay
                dold=d
            print(d*100)
